// Create a Database
CREATE DATABASE IF NOT EXISTS DBP;
...
SELECT * FROM SNOWFLKAE.SAMPLE_DATA.TPCH_SF1000.CUSTOMER;

// Observe time taken and no.of partitions scanned
SELECT * FROM PUBLIC.CUSTOMER_NONCLUSTER WHERE C_NATIONKEY=2; --  20 sec -- 420/420 mp scanned
SELECT * FROM PUBLIC.CUSTOMER_CLUSTER WHERE C_NATIONKEY=2; -- 11 sec -- 22/482 mp scanned

Select SYSTEM$CLUSTERING_INFORMATION('DBP.PUBLIC.CUSTOMER_NONCLUSTER'); -- Not Valid

Select SYSTEM$CLUSTERING_INFORMATION('DBP.PUBLIC.CUSTOMER_CLUSTER');--Only for the Cluster tables

set query_tag = 'VS CODE';
